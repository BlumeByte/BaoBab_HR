...
import '../../views/auth/employee_login_screen.dart';
import '../../views/auth/forgot_password_screen.dart';
import '../../views/auth/login_screen.dart';
import '../../views/auth/unauthorized_screen.dart';
import '../../views/auth/verify_email_screen.dart';
import '../../views/dashboard/employee_dashboard.dart';
import '../../views/dashboard/hr_dashboard.dart';
import '../../views/dashboard/super_dashboard.dart';
...
GoRouter createRouter(AuthProvider authProvider) {
  ...
  return GoRouter(
    ...
    redirect: (_, state) {
      ...
      final publicRoutes = {
        RouteNames.login,
        RouteNames.employeeLogin,
        RouteNames.forgotPassword,
        RouteNames.verifyEmail,
        RouteNames.unauthorized,
      };

      if (!authProvider.isLoggedIn && !publicRoutes.contains(location)) {
        return RouteNames.login;
      }

      if (authProvider.isLoggedIn && (location == RouteNames.login || location == RouteNames.employeeLogin)) {
        return homeByRole();
      }

      if (authProvider.isLoggedIn && !authProvider.isEmailVerified && location != RouteNames.verifyEmail) {
        return RouteNames.verifyEmail;
      }

      if (authProvider.isLoggedIn && !publicRoutes.contains(location) && !allowedForRole(location)) {
    ...
    routes: [
      ...
      GoRoute(path: RouteNames.employeeLogin, builder: (_, __) => const EmployeeLoginScreen()),
      GoRoute(path: RouteNames.forgotPassword, builder: (_, __) => const ForgotPasswordScreen()),
      GoRoute(path: RouteNames.verifyEmail, builder: (_, __) => const VerifyEmailScreen()),
      GoRoute(path: RouteNames.unauthorized, builder: (_, __) => const UnauthorizedScreen()),
      ShellRoute(
        builder: (_, __, child) => MainLayout(child: child),
        routes: [
          GoRoute(path: RouteNames.superDashboard, builder: (_, __) => const SuperDashboard()),
          GoRoute(path: RouteNames.hrDashboard, builder: (_, __) => const HrDashboard()),
          GoRoute(path: RouteNames.employeeDashboard, builder: (_, __) => const EmployeeDashboard()),
